...
- Tool: `tools/hal_crypto/` — `boolfunc.py` (truth tables, ANF, match tiers),
  ...
  `shiftreg.py` (chains walked by predecessor, so a tapped chain survives;
  operating-mode cofactor; coupling),
  `wordorder.py` (word membership and bit order off the carry chain, for
  netlists whose names carry nothing),
...
- Tests: `python -m unittest discover -s tools/hal_crypto -t tools -p "test_*.py"`
  (no HAL, 133 tests, ~30 s), registered with ctest as
