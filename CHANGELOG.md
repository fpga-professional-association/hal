...
* Plugins
  ...
  * Verilog parser and writer
    ...
    * fixed the Verilog writer silently merging two distinct nets (#60): HAL models the constant signals as the nets named `'0'` and `'1'`, which were escaped into `\'0'` and `\'1'` on write -- exactly how a net that genuinely carries such an escaped identifier (as fasm2bels emits) is written as well. A netlist holding both declared one wire for two nets, and re-parsing folded them into one. No identifier spelling maps back to `'0'`, as the parser deliberately keeps the backslash of `\'0'` to tell the two apart, so the constants are now written as the number literals they stand for: inline where nothing drives the net, and as a `wire <name> = 1'b0;` where a GND or VCC gate does, since the output pin of an instance may not be connected to a literal. The parser merges that wire back into the constant net it came from, name included, and escaped identifiers keep rendering as themselves
