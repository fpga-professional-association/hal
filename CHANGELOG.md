...
* Plugins
  ...
  * Verilog parser and writer
    ...
    * fixed the Verilog writer silently merging two distinct nets (#60): HAL models the constant signals as the nets named `'0'` and `'1'`, which were escaped into `\'0'` and `\'1'` on write -- exactly how a net that genuinely carries such an escaped identifier (as fasm2bels emits) is written as well. A netlist holding both declared one wire for two nets, and re-parsing folded them into one. The constants are now written as the number literals they stand for, which is what the parser turns back into these very nets, and the escaped identifiers keep rendering as themselves
