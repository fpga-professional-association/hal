...
def _select_gates(netlist, args, stems, reporter):
    """Resolve ``--module``/``--gate``/``--depth`` into a gate list.

    Shared by ``netlist_graph`` and ``dag`` so the two scope identically.
    ``stems`` is the ``(whole netlist, module, gate)`` triple of default output
    names; returns ``(gates, scope_description, output_stem)``.
    """
    if args.module is not None and args.gate is not None:
        raise SystemExit("hal_viz: --module and --gate are mutually exclusive")

    whole_stem, module_stem, gate_stem = stems
    stem = whole_stem
    if args.module is not None:
        ...
        stem = module_stem.format(module.get_id())
        if args.depth:
        ...
        stem = gate_stem.format(seed.get_id(), args.depth)
    else:
...
def cmd_netlist_graph(args, reporter):
    ...
    gates, scope, stem = _select_gates(
        netlist,
        args,
        ("netlist_graph", "module_{}_graph", "gate_{}_depth{}"),
        reporter,
    )
...
def cmd_dag(args, reporter):
    """Render the netlist as a topologically levelled directed graph."""
    hal_py = import_hal_py(args.hal_lib)
    load_all_plugins(hal_py)  # see cmd_netlist_graph: the parsers live in plugins
    netlist = load_netlist(hal_py, args.netlist, args.gate_library)

    gates, scope, stem = _select_gates(
        netlist, args, ("dag", "module_{}_dag", "gate_{}_depth{}_dag"), reporter
    )
    design = netlist.get_design_name() or os.path.basename(str(args.netlist))
    title = "{} - {} - levelled graph".format(design, scope)
    reporter.info("levelling the combinational graph: {}".format(title))

    view = build_dag_graph(
        gates,
        title=title,
        rankdir=args.rankdir,
        net_labels=args.net_labels,
        pin_labels=args.pin_labels,
        label_limit=args.label_limit,
        legend=not args.no_legend,
        level_labels=not args.no_level_labels,
        const_hub=args.const_hub,
        comment="generated by hal_viz {} (dag)".format(__version__),
    )

    warnings = []
    if view.has_cycles:
        message = (
            "{} gate(s) form a combinational loop that survives cutting the "
            "feedback at the registers; they are drawn highlighted: {}".format(
                len(view.cycle_gates), view.cycle_summary()
            )
        )
        warnings.append(message)
        reporter.warn(message)
    reporter.info(
        "{} gate(s), {} edge(s), {} level(s), {} edge(s) cut at a register, "
        "{} constant tie-off(s)".format(
            view.gate_count,
            view.edge_count,
            view.level_count,
            view.cut_count,
    ...
    base, fmt = _resolve_output(args.output, stem, args.format)
    dot_path, rendered = _emit(view.graph, base, fmt, args, reporter)
    ...
    if args.html:
        ...
        write_html_page(
            page,
            "{} - levelled netlist graph".format(design),
            svg_path=rendered if rendered and str(rendered).endswith(".svg") else None,
            caption="{}. Feedback is cut at every flip-flop and latch, so what "
            "is drawn left to right is the depth of the combinational core."
            .format(scope[:1].upper() + scope[1:]),
            stats=[
                ("gates", view.gate_count),
                ("edges", view.edge_count),
                ("levels", view.level_count),
                ("edges cut at a register", view.cut_count),
                ("constant tie-offs", view.tie_off_count),
            ],
            legend=LEGEND_ROWS,
            warnings=warnings,
            # a PNG/PDF render is not inlinable, so the page links whatever
            # artifact does exist rather than pretending there is none
            fallback=rendered or dot_path,
            footer="generated by hal_viz {} (dag) - this page is self-contained: "
            "inline SVG, inline CSS, no scripts, no external requests".format(
                __version__
            ),
        )
        reporter.info("wrote {}".format(page))

    _print_paths([dot_path, rendered, page])
    return 0
...
def cmd_clock_step(args, reporter):
    """Join a ``dag`` drawing to a ``hal_agilex trace`` into one interactive page.

    Needs neither HAL nor Graphviz: the drawing has already been rendered and
    the trace has already been recorded, and both are committed next to the
    walkthrough they illustrate.  The page it writes is self-contained.
    """
    svg_path = str(args.svg)
    dot_path = args.dot or os.path.splitext(svg_path)[0] + ".dot"
    for path, what in ((svg_path, "SVG"), (dot_path, "DOT source"), (args.trace, "trace")):
        if not os.path.isfile(str(path)):
            raise ClockStepError("{} not found: {}".format(what, path))

    with open(dot_path, encoding="utf-8") as handle:
        nodes, edges = parse_dag_dot(handle.read())
    with open(str(args.trace), encoding="utf-8") as handle:
        trace = json.load(handle)
    name_map = load_name_map(args.name_map) if args.name_map else None

    binding, report = bind_trace(nodes, edges, trace, name_map=name_map)
    if not report["nodes_bound"]:
        raise ClockStepError(
            "not one drawn gate matched a gate in the trace. The drawing is of "
            "{} and the trace is of {}; if the drawing is of an anonymised "
            "netlist, pass --name-map with the committed mapping.".format(
                os.path.basename(dot_path), trace.get("design", "?")
            )
        )
    reporter.info(
        "bound {}/{} gate(s) and {}/{} edge(s); {} tie-off(s), {} ambiguous "
        "edge(s)".format(
            report["nodes_bound"],
            report["nodes"] - report["ties"],
            report["edges_bound"],
            report["edges"],
            report["ties"],
    ...
    for name in report["nodes_unknown"]:
        reporter.warn("no trace values for drawn gate {!r}".format(name))

    design = trace.get("design") or "netlist"
    window = trace.get("window", {})
    title = args.title or "{} - clock-step DAG".format(design)
    caption = args.caption or (
        "Step the levelled graph one clock at a time. Colour is the value each "
        "net carries in the selected cycle; the shapes and dash patterns are "
        "the ones {} draws. Cycles {}-{} of a seeded run.".format(
            os.path.basename(dot_path), window.get("start", 0), window.get("end", 0)
        )
    )
    text = build_page(
        inline_svg(svg_path),
        ...
        title,
        caption=caption,
        blinded=bool(name_map),
        footer="generated by hal_viz {} (clock_step) from {} and {} - this page "
        "is self-contained: inline SVG, inline CSS, inline script, inline "
        "trace, no external requests".format(
            __version__, os.path.basename(svg_path), os.path.basename(str(args.trace))
        ),
    )
    path = _resolve_page_output(args.output, svg_path)
    _ensure_parent(path)
    write_page(path, text)
    reporter.info("wrote {} ({} cycle(s), {} bytes)".format(path, len(trace["frames"]), len(text)))
    _print_paths([path])
    return 0


def _resolve_page_output(output, svg_path):
    """Turn ``--output`` into the path of an .html file."""
    if not output:
        return os.path.abspath(os.path.splitext(svg_path)[0] + "_interactive.html")
    output = str(output)
    if output.endswith(("/", "\\")) or os.path.isdir(output) or os.path.basename(output) == "":
        return os.path.abspath(os.path.join(output, "dag_interactive.html"))
    if os.path.splitext(output)[1].lower() not in (".html", ".htm"):
        output += ".html"
    return os.path.abspath(output)
...
def _add_scope(parser):
    """The scope selection shared by ``netlist_graph`` and ``dag``."""
...
def _add_vocabulary(group):
    """The two switches over the shared visual vocabulary of every drawing."""
    group.add_argument(
        "--no-legend",
        action="store_true",
        help="omit the legend cluster that decodes shapes, edges and levels",
    )
    group.add_argument(
        "--const-hub",
        action="store_true",
        help="draw one shared GND/VCC node instead of a 0/1 tie-off stub per "
        "consuming edge (the old behaviour; a spider on a real netlist)",
    )
...
def build_parser():
    parser = argparse.ArgumentParser(
        ...
        epilog="Every command except 'report' and 'clock_step' requires a built "
        "HAL whose library directory is importable as hal_py (see --hal-lib / "
        "$HAL_PY_PATH); those two only read files that are already there. "
        "Paths of produced files are printed to stdout, one per line.",
    )
    ...
    _add_vocabulary(style)
    graph_parser.set_defaults(func=cmd_netlist_graph)

    # -- dag -----------------------------------------------------------------
    dag_parser = subparsers.add_parser(
        "dag",
        help="topologically levelled graph (combinational depth, left to right)",
        description="Render the netlist as a directed graph whose combinational "
        "core is levelled with Kahn's algorithm after the feedback is cut at "
        "every flip-flop and latch. Level 0 -- primary inputs, constant "
        "tie-offs and register outputs -- is on the far left and depth "
        "increases to the right. A real combinational loop is drawn "
        "highlighted and reported, never dropped. Same scoping as "
        "netlist_graph.",
    )
    _add_common(dag_parser, "dag")
    _add_scope(dag_parser)
    dag_style = dag_parser.add_argument_group("style")
    dag_style.add_argument(
        "--rankdir",
        choices=("LR", "RL", "TB", "BT"),
        default="LR",
        help="layout direction; LR puts level 0 on the left (default: %(default)s)",
    )
    dag_style.add_argument(
        "--net-labels", action="store_true", help="label edges with net names"
    )
    dag_style.add_argument(
        "--pin-labels", action="store_true", help="label edges with source/target pins"
    )
    dag_style.add_argument(
        "--no-level-labels",
        action="store_true",
        help="omit the 'level N' marker of each rank",
    )
    dag_style.add_argument(
        "--label-limit",
        type=int,
    ...
    # -- clock_step ------------------------------------------------------------
    clock_step_parser = subparsers.add_parser(
        "clock_step",
        help="interactive clock-step page over a dag drawing and a trace",
        description="Join a 'dag' SVG (plus the .dot it was rendered from) to "
        "a 'tools/hal_agilex trace' JSON and write one standalone HTML page "
        "that steps the design a clock at a time, colouring every net by the "
        "value it carries. Needs neither HAL nor Graphviz; the page opens "
        "offline with no external request.",
    )
    clock_step_parser.add_argument(
        "svg", metavar="DAG_SVG", help="the SVG written by 'hal_viz dag'"
    )
    clock_step_parser.add_argument(
        "--trace", required=True, metavar="JSON",
        help="the trace written by 'tools/hal_agilex trace'")
    clock_step_parser.add_argument(
        "--dot", metavar="PATH",
        help="the .dot the SVG was rendered from (default: the SVG's .dot sibling)")
    clock_step_parser.add_argument(
        "--name-map", metavar="JSON",
        help="mapping from the trace's gate names to the drawing's, for a "
        "walkthrough whose drawing is of an anonymised netlist; a flat "
        '{"real": "anon"} object or one with a "gates" member of that shape')
    clock_step_parser.add_argument(
        "-o", "--output", metavar="PATH",
        help="HTML file to write, or a directory when it ends with a separator "
        "(default: <svg>_interactive.html)")
    clock_step_parser.add_argument("--title", metavar="TEXT", help="page title")
    clock_step_parser.add_argument(
        "--caption", metavar="TEXT", help="one-line caption under the title")
    clock_step_parser.add_argument(
        "-q", "--quiet", action="store_true", help="suppress progress messages")
    clock_step_parser.add_argument(
        "--traceback", action="store_true",
        help="show the full Python traceback on error")
    clock_step_parser.set_defaults(func=cmd_clock_step)
