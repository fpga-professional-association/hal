...
| `resynthesis` | Gate/subgraph resynthesis and decomposition (e.g. into primitive gates) | `PL_RESYNTHESIS` (tri-state: `AUTO`/`ON`/`OFF`) | AUTO — built whenever `netlist_preprocessing` is, which links against it; `OFF` builds `netlist_preprocessing` without its resynthesis-based mux optimizations | `cmake/hal_resynthesis_option.cmake`, included from `plugins/CMakeLists.txt:15` (option moved out of the plugin's own `CMakeLists.txt` because `netlist_preprocessing` has to know whether it exists — see `plugins/resynthesis/CMakeLists.txt:1-3`) |
...
| `simulator` | Event-based gate-level netlist simulator (`hal_simulator`), a `netlist_simulator_controller` orchestration layer, and an optional Verilator-backed engine (`verilator/`) | `PL_SIMULATOR` (umbrella), `PL_NETLIST_SIMULATOR` (engine), `PL_NETLIST_SIMULATOR_CONTROLLER` (orchestration), `PL_VERILATOR` (Verilator backend) | all OFF; the two sub-plugins are also built whenever `PL_SIMULATOR` is on (`... OR PL_SIMULATOR OR BUILD_ALL_PLUGINS`) | `plugins/simulator/CMakeLists.txt:1`, `plugins/simulator/hal_simulator/CMakeLists.txt:1`, `plugins/simulator/netlist_simulator_controller/CMakeLists.txt:1`, `plugins/simulator/verilator/CMakeLists.txt:1` |
...
**One Intel/Altera library is bundled, and its coverage is deliberately narrow.**
`AGILEX_TENNM.hgl` is generated by `tools/hal_agilex library` and covers exactly two Agilex 3
primitives — `tennm_lcell_comb` and `tennm_ff` — in one configuration each. `tennm_lcell_comb`
intentionally carries **no** Boolean function: its behaviour comes from a `lut_mask` decoded per
instance, not from the library. No Stratix/Cyclone/Arria `.hgl` ships, and no other Agilex
primitive is covered.

Read that as a floor, not a ceiling: a netlist that instantiates any Agilex primitive outside
those two will not elaborate. Parsing support for the HDL is not device support, and a bundled
library for two primitives is not a device family.
...
- **Agilex / Quartus is only narrowly supported.** `AGILEX_TENNM.hgl` now ships (see 2b) and
  `tools/hal_agilex` can import and elaborate Quartus `.vo` exports, but that covers two
  primitives in one configuration each. `module_identification` still has **no** dispatch path
  for `AGILEX_TENNM`, so arithmetic recognition does not run on Agilex netlists at all —
  tracked in [#55](https://github.com/fpga-professional-association/hal/issues/55). Simulation
  semantics are validated only by the bounded reference-model checks in
  `examples/agilex3_walkthroughs/`, not in general.
