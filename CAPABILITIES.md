...
| `resynthesis` | Gate/subgraph resynthesis and decomposition (e.g. into primitive gates) | `PL_RESYNTHESIS` (tri-state: `AUTO`/`ON`/`OFF`) | AUTO — built whenever `netlist_preprocessing` is, which links against it; `OFF` builds `netlist_preprocessing` without its resynthesis-based mux optimizations | `plugins/CMakeLists.txt:23-48` (option moved out of the plugin's own `CMakeLists.txt` because `netlist_preprocessing` has to know whether it exists — see `plugins/resynthesis/CMakeLists.txt:1-3`) |
